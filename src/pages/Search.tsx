  ...
  useEffect(() => {
    const applyGenderFilter = async () => {
      try {
        ...
        if (!session?.user) { loadHostels(); return; }

        const { data: roleData } = await supabase
          .from("user_roles").select("role")
          .eq("user_id", session.user.id).maybeSingle();

        if (roleData?.role !== "student") { loadHostels(); return; }
        ...
        if (profile?.gender) {
          ...
          const genderFilters = { ...defaultFilters, hostelType };
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-2">
                <p className="text-muted-foreground">
                  {hasSearched ? "No hostels found matching your criteria." : "No hostels found"}
                </p>
              </div>
            ) : (
